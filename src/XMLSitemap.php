...
namespace Wearejust\KirbyXMLSitemap;
...
class XMLSitemap
{
    ...
    {
        if ('true' == self::getConfigurationForKey('disable')) {
            return false;
        }

        if ('false' == \kirby()->site()->content()->get('xmlsitemap')) {
            return false;
        }
    ...
    {
        $f = null;
        if (true == static::getConfigurationForKey('x-shimAssets')) {
    ...
    {
        static::$debug = $debug && null !== \kirby()->option('debug') && true == \kirby()->option('debug');
        static::pickupOptions();

        $tbeg = \microtime(true);

        // if cacheTTL disabled or nocache set
        if (null == static::$optionCACHE || '' == static::$optionCACHE || (false != $nocache && true == $debug)) {
    ...
    private static function generateSitemap(Pages $p, bool $debug = false): string
    {
        ...
        if (true == $debug) {
            ...
            $r .= '<!--                      addPages = '.(null != static::getClosureForKey('addpages') ? 'Closure' : 'null')." -->\n";
        ...
        static::addComment($r, 'Kl = '.(false == $license ? 'n' : 'y'));
        ...
        $r .= "</urlset>\n";
        $r .= "<!-- Sitemap generated using https://github.com/wearejust/kirby-xmlsitemap -->\n";
    ...
    private static function addPagesToSitemapFromClosure(string &$r, string $langcode = null): void
    {
        ...
        if (null == $c) {
            ...
            if ('Kirby\Cms\Pages' == \get_class($c)) {
        ...
        foreach ($pages as $p) {
            ...
            if (null == $langcode) {
                ...
                if ('--' == $langcode) {
                    ...
                    if (true == static::$optionNOTRA && !$p->translation($langcode)->exists()) {
            ...
            if ('unlisted' == $p->status() && !$p->isHomePage()) {
            ...
            if ('true' == $p->content()->excludefromxmlsitemap()) {
            ...
            $r .= "<url>\n";
            if (null == $langcode) { // single-language
                $r .= '  <loc>'.$p->url().'</loc>'."\n";
            } else {
                ...
                foreach (\kirby()->languages() as $l) {
                    if (true == static::$optionNOTRA && !$p->translation($l->code())->exists()) {
